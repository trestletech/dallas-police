...
getDay <- function(year, month, day, downloadDir = "../download/"){ #5s per day.
...
yr14.old <- readRDS("2014.Rds")
# lat/lon NA = 281751 (maybe 4/9/2015?)
# sum(is.na(yr14$Lat)) = 257749 as of 7/7/2015
yr14 <- getYear(2014)
#summary(yr14  %>% mutate(day=yday(parse_date_time(DateTime, orders="b d! Y! I!:M!p"))) %>% group_by(day) %>% summarize(count=n()))

# Sometimes mapquest would point to the center of dallas if it didn't know an address. Find those entries.
repeatedLatLong <- yr14 %>% 
  group_by(Lat, Long) %>% 
  summarize(count=n(), diff=length(unique(Street))) %>% 
  ungroup() %>% 
  filter(diff > 100) # more than 100 different streets/intersections corresponding to same location.

clean14 <- yr14 %>% filter(! Lat %in% repeatedLatLong$Lat & !Long %in% repeatedLatLong$Long)



mostCalls <- clean14 %>% group_by(Block, Street) %>% summarize(count=n(), Lat=head(Lat,n =1), Long=head(Long, n=1)) %>% ungroup() %>% arrange(desc(count)) %>% head(n=50)
mostCalls %>% ggvis(~Lat, ~Long) %>% layer_points
library(leaflet)

qpal <- colorQuantile("Reds", mostCalls$count, n = 5)

mostCalls %>% 
  mutate(severity = log(count, 1500), name=paste(Block, Street)) %>% 
  leaflet() %>% 
  addProviderTiles("CartoDB.Positron") %>%  
  addCircles(~Long, ~Lat, radius=125, fillOpacity=~severity, color=~qpal(count), popup=~name)


latlon <- clean14 %>% 
  dplyr::select(Lat, Long) %>% 
  filter(Lat > quantile(Lat, .02) & Lat < quantile(Lat, .98)) %>% 
  filter(Long > quantile(Long, .02) & Long < quantile(Long, .98))
kde <- kde2d(latlon$Long, latlon$Lat, n=320)
image(kde, col=topo.colors(12))
ras <- raster(kde)

pal <- colorNumeric(c("#DDDDDD00", "#41B6C4AA", "#0C2C84FF"), values(ras),
                    na.color = "transparent", alpha=TRUE)
clean14 %>% 
  leaflet() %>% 
  addProviderTiles("CartoDB.Positron") %>%  
  addRasterImage(ras, colors=pal, opacity = 0.7)



# library(ggvis)
# yr14  %>% mutate(day=yday(parse_date_time(DateTime, orders="b d! Y! I!:M!p"))) %>% group_by(day) %>% summarize(count=n())  %>% ggvis(~count)  %>% ggvis::layer_histograms()
